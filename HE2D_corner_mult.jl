...
xW  = 0;
xS  = 0;
xIN = 8*lambda_0;  # Width.
xN  = 11*lambda_0;  # Width.
xIE = 8*lambda_0;  # Location of interface.
xE  = 11*lambda_0;  # Location of ABC.
nLE = (xE-xIE)/hx;
nLN = (xN-xIN)/hy;
#P = 7;
...
radius = 20;   					# Radius of the Gaussian pulse.
a = log(10^6)/(radius*radius);  # Amplitude.
xstar =  20;					# Location in x-direction.
ystar =  32;					# Location in y-direction.
...
I7e  =  I7(r,hx,gj,pref,elref);
I8e  =  I8(r,hx,gj,pref,elref);
...
FE = spzeros(Complex{Float64},Ndof,1);
FN = spzeros(Complex{Float64},Ndof,1);
#
for l = 1 : Nel
	for i = 1 : (r+1)^2
...
for l = 1 : Nel
	...
	felE = zeros(Complex,(r+1)^2,1);
	felN = zeros(Complex,(r+1)^2,1);
	Fel  = zeros(Complex,(r+1)^2,1);
	FelE = zeros(Complex,(r+1)^2,1);
	FelN = zeros(Complex,(r+1)^2,1);
	for m = 1 : (r+1)^2
		ind1 = Int(DOF[Int(el[l,m])]);
#		if ind1==0  && p[el[l,m],1] <= 20 && p[el[l,m],2] <= 20
		if ind1!=0
#			Gaussian			
...
#			fel[m] = 0.25*1im*(besselj0.(kn*dist) + 1im*bessely0.(kn*dist));
			if ((xIE - p[el[l,1],1]) < 1e-6*hx)					#
				felE[m] = 0.25*1im*(besselj0.(kn*dist) + 1im*bessely0.(kn*dist));
			end		
			if ((xIN - p[el[l,1],2]) < 1e-6*hy)					#
				felN[m] = 0.25*1im*(besselj0.(kn*dist) + 1im*bessely0.(kn*dist));
			end													#
											#
#			fel[m] = (1-p[el[l,m],1]^2/400)*(1-p[el[l,m],2]^2/400);
...
#			fel[m] = cos.(ky*p[el[l,m],2]);
	...
	Fel   = -(I2e-kn*kn*I1e-1im*kn*I3e-1im*kn*I7e)*fel;
	FelE  = -(I2e-kn*kn*I1e-1im*kn*I3e-1im*kn*I7e)*felE;
	FelN  = -(I2e-kn*kn*I1e-1im*kn*I3e-1im*kn*I7e)*felN;
...
#	Fel = I1e*fel;
	for k = 1 : (r+1)^2
		...
		if ind1 != 0
			 F[ind1] =  F[ind1] +  Fel[k];
			FE[ind1] = FE[ind1] + FelE[k];
			FN[ind1] = FN[ind1] + FelN[k];
		end
	end
end
...
if SaveSol == 1
	...
	pf = p[findind2,:];
	for i = 1 : size(pf,1)
		if p[findind2[i],1] <= 3*lambda_0 && p[findind2[i],2] <= 3*lambda_0
    		for j = 1 : 3
		        print(fp,pf[i,j]," ")
		    end
	...
	g = besselj0.(kn*dist) + 1im*bessely0.(kn*dist);
	g = 0.25*1im*g;
...
#   g= exp.(1im*kn*dist)./(4*pi*dist);
	...
	for tt = 1 : size(pf,1)
		if p[findind2[tt],1] <= 3*lambda_0 && p[findind2[tt],2] <= 3*lambda_0
	        print(fr ,real(u[tt])," ")
    	    print(fi ,imag(u[tt])," ")
	        print(fre,real(g[tt])," ")
    	    print(fie,imag(g[tt])," ")
